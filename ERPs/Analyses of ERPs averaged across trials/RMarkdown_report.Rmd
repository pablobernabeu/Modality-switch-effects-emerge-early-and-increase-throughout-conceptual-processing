...
---
title: 'General statistical analysis'
---
...
# Print numbers directly, without exponents
options(scipen=999)
...
# Peak waveforms and latencies per group and time window

# Time Window 1

dat = aggreg_EEG[aggreg_EEG$timewindow=='Window 1' & aggreg_EEG$RT.based_Groups=='Quick', ]
dat[which.max(abs(dat$microvolts)),]

dat = aggreg_EEG[aggreg_EEG$timewindow=='Window 1' & aggreg_EEG$RT.based_Groups=='Slow', ]
dat[which.max(abs(dat$microvolts)),]


# Time Window 2

dat = aggreg_EEG[aggreg_EEG$timewindow=='Window 2' & aggreg_EEG$RT.based_Groups=='Quick', ]
dat[which.max(abs(dat$microvolts)),]

dat = aggreg_EEG[aggreg_EEG$timewindow=='Window 2' & aggreg_EEG$RT.based_Groups=='Slow', ]
dat[which.max(abs(dat$microvolts)),]


# Time Window 3

dat = aggreg_EEG[aggreg_EEG$timewindow=='Window 3' & aggreg_EEG$RT.based_Groups=='Quick', ]
dat[which.max(abs(dat$microvolts)),]

dat = aggreg_EEG[aggreg_EEG$timewindow=='Window 3' & aggreg_EEG$RT.based_Groups=='Slow', ]
dat[which.max(abs(dat$microvolts)),]


# Time Window 4

dat = aggreg_EEG[aggreg_EEG$timewindow=='Window 4' & aggreg_EEG$RT.based_Groups=='Quick', ]
dat[which.max(abs(dat$microvolts)),]

dat = aggreg_EEG[aggreg_EEG$timewindow=='Window 4' & aggreg_EEG$RT.based_Groups=='Slow', ]
dat[which.max(abs(dat$microvolts)),]
...
summaryBy(microvolts ~ condition, FUN=c(mean, sd), EEG[!is.na(EEG$location) & !is.na(EEG$RT.based_Groups) &
	!is.na(as.numeric(EEG$time)>=0),])



# Mean and SD of the Switch conditions per group, time window and brain area

summaryBy(microvolts ~ c(RT.based_Groups, location, condition), FUN=c(mean, sd),
EEG[EEG$timewindow=='Window 1' & !is.na(EEG$location) & !is.na(EEG$RT.based_Groups),])

summaryBy(microvolts ~ c(RT.based_Groups, location, condition), FUN=c(mean, sd),
EEG[EEG$timewindow=='Window 2' & !is.na(EEG$location) & !is.na(EEG$RT.based_Groups),])

summaryBy(microvolts ~ c(RT.based_Groups, location, condition), FUN=c(mean, sd),
EEG[EEG$timewindow=='Window 3' & !is.na(EEG$location) & !is.na(EEG$RT.based_Groups),])

summaryBy(microvolts ~ c(RT.based_Groups, location, condition), FUN=c(mean, sd),
EEG[EEG$timewindow=='Window 4' & !is.na(EEG$location) & !is.na(EEG$RT.based_Groups),])
...
# Perhaps transform DV. First check several methods. Note: +74 is
# added in order to turn DV positive, avoiding non-values. Note
# also that the transformation is checked on Time Window 3 because
# it contains the most relevant ERP component, N400.

# Window 3
EEG.window3 <- cbind(EEG.window3, EEG.window3$microvolts^2)
EEG.window3 <- cbind(EEG.window3, sqrt(EEG.window3$microvolts +74))
EEG.window3 <- cbind(EEG.window3, log(EEG.window3$microvolts +74))
EEG.window3 <- cbind(EEG.window3, 1/(EEG.window3$microvolts +74))
EEG.window3 <- cbind(EEG.window3, 1/sqrt(EEG.window3$microvolts +74))
EEG.window3$sqmicrovolts <- EEG.window3$microvolts^2
EEG.window3$sqrtmicrovolts <- sqrt(EEG.window3$microvolts +74)
EEG.window3$logmicrovolts <- log(EEG.window3$microvolts +74)
EEG.window3$recmicrovolts <- 1/(EEG.window3$microvolts +74)
EEG.window3$recsqrtmicrovolts <- 1/sqrt(EEG.window3$microvolts +74)


# First, check residuals w/ original, untransformed variable
orig = lmer(microvolts ~ (1|participant)+ time+ (1|participant: time) 
	+ (1|electrode) +(1|electrode: participant)+ (1|condition: 
	participant)+ (1| RT.based_Groups: time)+ (1| electrode: participant: 
	condition)+ location+ condition + RT.based_Groups : location,
 	data=EEG.window3,
...
sq_t = lmer(sqmicrovolts ~ (1|participant)+ time+ (1|participant: time) 
	+ (1|electrode) +(1|electrode: participant)+ (1|condition: 
	participant)+ (1| RT.based_Groups: time)+ (1| electrode: participant: 
	condition)+ location+ condition + RT.based_Groups : location,
 	data=EEG.window3,
...
qqnorm(resid(sq_t))
# Plot of Residuals vs. Fitted values:
plot(fitted(sq_t),resid(sq_t),xlab='Fitted Values', ylab='Residuals'); 
abline(h=0)

# Third, check residuals when square root transformation is applied on DV
sqrt_t = lmer(sqrtmicrovolts ~ (1|participant)+ time+ (1|participant: time) 
	+ (1|electrode) +(1|electrode: participant)+ (1|condition: 
	participant)+ (1| RT.based_Groups: time)+ (1| electrode: participant: 
	condition)+ location+ condition + RT.based_Groups : location,
 	data=EEG.window3,
...
qqnorm(resid(sqrt_t))
# Plot of Residuals vs. Fitted values:
plot(fitted(sqrt_t),resid(sqrt_t),xlab='Fitted Values', ylab='Residuals'); 
abline(h=0)

# Fourth, check residuals when log transformation is applied on DV
log_t = lmer(logmicrovolts ~ (1|participant)+ time+ (1|participant: time) 
	+ (1|electrode) +(1|electrode: participant)+ (1|condition: 
	participant)+ (1| RT.based_Groups: time)+ (1| electrode: participant: 
	condition)+ location+ condition + RT.based_Groups : location,
 	data=EEG.window3,
...
qqnorm(resid(log_t))
# Plot of Residuals vs. Fitted values:
plot(fitted(log_t),resid(log_t),xlab='Fitted Values', ylab='Residuals'); 
abline(h=0)

# Fifth, check residuals when reciprocal transformation is applied on DV
rec_t = lmer(recmicrovolts ~ (1|participant)+ time+ (1|participant: time) 
	+ (1|electrode) +(1|electrode: participant)+ (1|condition: 
	participant)+ (1| RT.based_Groups: time)+ (1| electrode: participant: 
	condition)+ location+ condition + RT.based_Groups : location,
 	data=EEG.window3,
...
qqnorm(resid(rec_t))
# Plot of Residuals vs. Fitted values:
plot(fitted(rec_t),resid(rec_t),xlab='Fitted Values', ylab='Residuals'); 
abline(h=0)

# Sixth, check residuals when reciprocal of the square root transformation is applied on DV
recsq_t = lmer(recsqrtmicrovolts ~ (1|participant)+ time+ (1|participant: time) 
	+ (1|electrode) +(1|electrode: participant)+ (1|condition: 
	participant)+ (1| RT.based_Groups: time)+ (1| electrode: participant: 
	condition)+ location+ condition + RT.based_Groups : location,
 	data=EEG.window3,
...
# Residuals diagnostics:
# Normal Q-Q plot of residuals:
qqnorm(resid(recsq_t))
# Plot of Residuals vs. Fitted values:
plot(fitted(recsq_t),resid(recsq_t),xlab='Fitted Values', ylab='Residuals'); 
abline(h=0)
...
# models in R. Behavior Research Methods, 49: 1494. https://doi.org/10.3758/s13428-016-0809-y.
...
# Tested on all remaining time windows; not converging for any.
...
# Tested on all remaining time windows; converging for Time Window 3.
saveRDS(m.w3, 'LME model Time Window 3.rds', compress='xz')		# Model run with data: EEG[EEG$timewindow == "Window 3", ]
...
# Tested on all remaining time windows; converging for Time Windows 1 and 4.
saveRDS(m.w1, 'LME model Time Window 1.rds', compress='xz')		# Model run with data: EEG[EEG$timewindow == "Window 1", ]
saveRDS(m.w4, 'LME model Time Window 4.rds', compress='xz')		# Model run with data: EEG[EEG$timewindow == "Window 4", ]
...
# Converging for Time Window 2
saveRDS(m.w2, 'LME model Time Window 2.rds', compress='xz')		# Model run with data: EEG[EEG$timewindow == "Window 2", ]
...
# Residuals and goodness-of-fit measures for each time window. Models are fit using lmer() because the
# functions used require that.


# Time Window 1

lmer_m.w1 = lmer(microvolts ~
	(1| time) + (1| electrode) + (1| RT.based_Groups : participant) +
	(condition | participant) +
	RT.based_Groups * condition * location,
	data = EEG[EEG$timewindow=='Window 1',],
	control = lmerControl(optCtrl=list(maxfun=10000000)))
...
#png(file="Time window 4, posterior distributions of fixed effects.png", units="in", width=6, height=6, res=500)
...
lmer_m.w2 = lmer(microvolts ~
	(1| time) + (1| electrode) + (condition | participant) +
	RT.based_Groups * condition * location,
	data = EEG[EEG$timewindow=='Window 2',],
	control = lmerControl(optCtrl=list(maxfun=10000000)))
...
lmer_m.w3 = lmer(microvolts ~
	(1| time) + (1| electrode) + (1| location : electrode) +
	(condition | participant) +
	RT.based_Groups * condition * location,
	data = EEG[EEG$timewindow=='Window 3',],
	control = lmerControl(optCtrl=list(maxfun=10000000)))
...
lmer_m.w4 = lmer(microvolts ~
	(1| time) + (1| electrode) + (1| RT.based_Groups : participant) +
	(condition | participant) +
	RT.based_Groups * condition * location,
	data = EEG[EEG$timewindow=='Window 4',],
	control = lmerControl(optCtrl=list(maxfun=10000000)))
...
Estimate = as.vector(coefficients(summary(tw1))[1:length(rownames(coefficients(summary(tw1)))), 'Estimate'])
SE = as.vector(coefficients(summary(tw1))[1:length(rownames(coefficients(summary(tw1)))), 'Std. Error'])
Lower_CI = Estimate - 1.96 * SE
Upper_CI = Estimate + 1.96 * SE
t = as.vector(coefficients(summary(tw1))[1:length(rownames(coefficients(summary(tw1)))), 't value'])
df = as.vector(coefficients(summary(tw1))[1:length(rownames(coefficients(summary(tw1)))), 'df'])
t_p = as.vector(coefficients(summary(tw1))[1:length(rownames(coefficients(summary(tw1)))), 'Pr(>|t|)'])

results_tw1_t = data.frame(Time.window, Statistic, Variable, Estimate, SE, Lower_CI, Upper_CI, t, df, t_p)
...
Estimate = as.vector(coefficients(summary(tw2))[1:length(rownames(coefficients(summary(tw2)))), 'Estimate'])
SE = as.vector(coefficients(summary(tw2))[1:length(rownames(coefficients(summary(tw2)))), 'Std. Error'])
Lower_CI = Estimate - 1.96 * SE
Upper_CI = Estimate + 1.96 * SE
t = as.vector(coefficients(summary(tw2))[1:length(rownames(coefficients(summary(tw2)))), 't value'])
df = as.vector(coefficients(summary(tw2))[1:length(rownames(coefficients(summary(tw2)))), 'df'])
t_p = as.vector(coefficients(summary(tw2))[1:length(rownames(coefficients(summary(tw2)))), 'Pr(>|t|)'])

results_tw2_t = data.frame(Time.window, Statistic, Variable, Estimate, SE, Lower_CI, Upper_CI, t, df, t_p)
...
Estimate = as.vector(coefficients(summary(tw3))[1:length(rownames(coefficients(summary(tw3)))), 'Estimate'])
SE = as.vector(coefficients(summary(tw3))[1:length(rownames(coefficients(summary(tw3)))), 'Std. Error'])
Lower_CI = Estimate - 1.96 * SE
Upper_CI = Estimate + 1.96 * SE
t = as.vector(coefficients(summary(tw3))[1:length(rownames(coefficients(summary(tw3)))), 't value'])
df = as.vector(coefficients(summary(tw3))[1:length(rownames(coefficients(summary(tw3)))), 'df'])
t_p = as.vector(coefficients(summary(tw3))[1:length(rownames(coefficients(summary(tw3)))), 'Pr(>|t|)'])

results_tw3_t = data.frame(Time.window, Statistic, Variable, Estimate, SE, Lower_CI, Upper_CI, t, df, t_p)
...
Estimate = as.vector(coefficients(summary(tw4))[1:length(rownames(coefficients(summary(tw4)))), 'Estimate'])
SE = as.vector(coefficients(summary(tw4))[1:length(rownames(coefficients(summary(tw4)))), 'Std. Error'])
Lower_CI = Estimate - 1.96 * SE
Upper_CI = Estimate + 1.96 * SE
t = as.vector(coefficients(summary(tw4))[1:length(rownames(coefficients(summary(tw4)))), 't value'])
df = as.vector(coefficients(summary(tw4))[1:length(rownames(coefficients(summary(tw4)))), 'df'])
t_p = as.vector(coefficients(summary(tw4))[1:length(rownames(coefficients(summary(tw4)))), 'Pr(>|t|)'])

results_tw4_t = data.frame(Time.window, Statistic, Variable, Estimate, SE, Lower_CI, Upper_CI, t, df, t_p)
...
# Adjust p values via Holm-Bonferroni. It is done per variable through the time windows. Thus, the lowest
# p-value in the four time windows is multiplied by 4, the next p-value is multiplied by 3, the next by 2, 
# and the highest p-value is left as is. This holds as long as p = 1 is not reached before the end--in which 
# case all subsequent p-values are left at 1. This is the standard Holm-Bonferroni method.
...
results$F_p.asterisks = 
	ifelse(results$'Holm-Bonferroni-corrected_F_p' > .001 & results$'Holm-Bonferroni-corrected_F_p' < .01, '**',
	results$F_p.asterisks)

results$F_p.asterisks = 
	ifelse(results$'Holm-Bonferroni-corrected_F_p' > .01 & results$'Holm-Bonferroni-corrected_F_p' < .05, '*',
	results$F_p.asterisks)
...
results$t_p.asterisks = 
	ifelse(results$'Holm-Bonferroni-corrected_t_p' > .001 & results$'Holm-Bonferroni-corrected_t_p' < .01, '**',
	results$t_p.asterisks)

results$t_p.asterisks = 
	ifelse(results$'Holm-Bonferroni-corrected_t_p' > .01 & results$'Holm-Bonferroni-corrected_t_p' < .05, '*',
	results$t_p.asterisks)
